...
jQuery(document).ready(function() {
    ...
    */
    $('.register form').submit(function(){
		...
		$(this).find("label[for='respass']").html('resPass');
        ////
       
        var username = $(this).find('input#username').val();
        var email = $(this).find('input#email').val();
        var password = $(this).find('input#password').val();
        var password = $(this).find('input#respass').val();
	   ...
	   if(password == '') {
            $(this).find("label[for='respass']").append("<span style='display:none' class='red'> - Please enter a valid respass.</span>");
            $(this).find("label[for='respass'] span").fadeIn('medium');
            return false;
        }
